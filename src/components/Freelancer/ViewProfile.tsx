...
function ViewProfile() {
	...
	const fetchStepQuestions = async () => {
		try {
			...
			if (details.step10.user_details) {
				...
				setStarRating(details.step10.star_rating)
			}
		} catch (error) {
	...
	const handleResumeClick = (e: React.MouseEvent, resumePath: string) => {
		const active_user = getUserInfo('id');
		if (!active_user) {
	...
	return (
		<>
			...
			<AuthLayout >
									...
									<div className="user-top-info-sec">
										<div className="pr-user-badge d-flex flex-wrap align-items-center">
											<div className="pr-badge-image">
												<img loading="lazy" className="img-fluid" src={profileimage} alt="" title="" />
												{showControl() && (
													<button className="edit_Link" type="button" onClick={() => setImageModal(true)}>
														<img loading="lazy" className="img-fluid" src="/assets/images/edit-light-icon.svg" alt="" title="" />
													</button>
												)}
											</div>
											<div className="pr-user-badge-text">
												...
												<div className="pr-fl-rating d-flex align-items-center">
													<span className="fl-rating-text">Excellent {starRating > 0 && Number(starRating)}</span> <span className="fl-rating-icon"><StarRating rating={Number(starRating)} /></span>
												</div>
												<div className="user-badge-quality d-flex align-items-center">
													<span><img loading="lazy" className="img-fluid" src="/assets/images/top-pro-icon.svg" /></span>Top Pro
												</div>
												<div className="pr-share-btns d-flex align-items-center flex-wrap">
													<button className="share-btns" type="button" onClick={() => setShareModal(true)}><img loading="lazy" className="img-fluid" src="/assets/images/share-icon.svg" alt="" title="" /> Share</button>
												</div>
											</div>
										</div>
											...
											<div className="colm-6">
												<div className="all-profile-info-sec left-info-contents">
													...
													<div className="profile-resume-btns">
														{resume.id > 0 && (
															<Link className="view-resume-btns outline-border-btns" onClick={(e) => handleResumeClick(e, resume.resume_path)} to={resume.resume_path} target={'_blank'}> View Resume</Link>
														)}
														{showControl() && (
														...
														<div className="icon-title-wrapper-items d-flex align-items-center justify-space-between">
															...
															{showControl() && (
																<button className="plus_link outline-icon-btn" type="button" onClick={() => setExperianceModal(true)}>
																	<img loading="lazy" className="img-fluid" src="/assets/images/plus-pink-icon.svg" alt="" title="" />
																</button>
															)}
														</div>
																	...
																	{experiances.map(experiance => (
																		<li key={experiance.id} className="d-flex justify-space-between">
																			<div>
																				...
																				<div className="text-light-on-info">{experiance.start_month + '-' + experiance.start_year} {(experiance?.end_month && experiance?.end_year) ? '/' + experiance.end_month + '-' + experiance.end_year : ''}
																				</div>
																			...
																			{showControl() && (
																				<div className="d-flex two-icon-btns">
																					...
																					<button className="delete_link outline-icon-btn" type="button" onClick={() => handleDelete(experiance.id, 'experience')}>
																						<img loading="lazy" className="img-fluid" src="/assets/images/delete-color-icon.svg" alt="" title="" />
																					</button>
																				</div>
																			)}
																		</li>
									...
									<div className="pr-sidebar-boxs">
										...
										<div className="pr-gurantee-boxs">
											<div className="pr-gurantee-img-boxs d-flex align-items-center">
												<span className="gurantee-icon"> <img loading="lazy" className="img-fluid" src="/assets/images/gurantee-icon.svg" alt="" title="" /></span>
												<h3>Our Guarantee</h3>
											</div>
										...
										<div className="overview-content-div">
											...
											<div className="overview-listing">
												<ul>
													<li className="d-flex align-items-center pr-listing"><span className="over-list-icon"><img loading="lazy" className="img-fluid" src="/assets/images/current-pro-icon.svg" alt="" title="" /></span> <span className="over-list-text">Current top pro</span></li>
													<li className="d-flex align-items-center pr-listing"><span className="over-list-icon"><img loading="lazy" className="img-fluid" src="/assets/images/trophy-icon.svg" alt="" title="" /></span> <span className="over-list-text">Hire 171 times</span></li>
													<li className="d-flex align-items-center pr-listing"><span className="over-list-icon"><img loading="lazy" className="img-fluid" src="/assets/images/over-location-icon.svg" alt="" title="" /></span> <span className="over-list-text">6 Similar jobs near you</span></li>
													<li className="d-flex align-items-center pr-listing"><span className="over-list-icon"><img loading="lazy" className="img-fluid" src="/assets/images/user-checked-icon.svg" alt="" title="" /></span> <span className="over-list-text">Background Checked</span></li>
													<li className="d-flex align-items-center pr-listing"><span className="over-list-icon"><img loading="lazy" className="img-fluid" src="/assets/images/employ-icon.svg" alt="" title="" /></span> <span className="over-list-text">1 Employ</span></li>
													<li className="d-flex align-items-center pr-listing"><span className="over-list-icon"><img loading="lazy" className="img-fluid" src="/assets/images/time-line-icon.svg" alt="" title="" /></span> <span className="over-list-text">2 years in bussiness</span></li>
												</ul>
											</div>
										</div>
									</div>
				...
				<ProfileSocialShareModal url={currentUrl} title={profiletitle} description={aboutyourself} isOpen={sharemodal} onClose={() => { setShareModal(false); setStateChangeTrigger(true); }} />
				...
				<EditProfileImageModal isOpen={imagemodal} onClose={() => { setImageModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Resume Modal*/}
				<UploadResumeModal isOpen={resumemodal} onClose={() => { setResumeModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Title Modal*/}
				<EditProfileTitleModal isOpen={profiletitlemodal} onClose={() => { setProfileTitleModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Bio Modal*/}
				<EditProfileBioModal isOpen={profilebiomodal} onClose={() => { setProfileBioModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Skills Modal*/}
				<EditSkillsModal isOpen={skillmodal} onClose={() => { setSkillModal(false) }} />
				{/*Edit User Subcategory Modal*/}
				<EditSubCategoryModal isOpen={categorymodal} onClose={() => { setCategoryModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Profile Modal*/}
				<EditProfileModal isOpen={profilemodal} onClose={() => { setProfileModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Hourly Rate Modal*/}
				<EditHourlyRateModal isOpen={hourlyratemodal} onClose={() => { setHourlyRateModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Language Modal*/}
				<EditLanguageModal selected={editlanguage} isOpen={languagemodal} onClose={() => { setLanguageModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Qualifiaction Modal*/}
				<AddQualificationModal selected={editqualification} isOpen={qualificationmodal} onClose={() => { setQualificationModal(false); setStateChangeTrigger(true); }} />
				{/*Edit User Experiance Modal*/}
				<AddExperianceModal selected={editexperiance} isOpen={experiancemodal} onClose={() => { setExperianceModal(false); setStateChangeTrigger(true); }} />
			</AuthLayout>
